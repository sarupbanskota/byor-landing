...
function Hero({title, subtitle, testimonial, dark, hideCourses, hideLogos}) {
  ...
  return (
    <div className={dark ? "bg-radial-at-t from-black to-gray-900" : "bg-radial-at-b from-teal-400 via-lime-50 to-white"}>
      <div className="relative overflow-hidden">
        <div className="relative pt-6 pb-4 md:pb-16">
          <div className="mt-8 mx-auto max-w-7xl px-4 sm:mt-8 sm:px-6">
            <div className="text-center">
              <h1 className={`text-4xl tracking-tight font-extrabold sm:text-5xl md:text-6xl ${dark ? darkTitleColor: ""}`}>
                {title}
              </h1>
              <p className={`mt-3 max-w-md mx-auto text-lg sm:text-xl md:mt-5 md:text-2xl md:max-w-3xl ${dark ? "text-white": ""}`}>
                {subtitle}
              </p>
              {/* <div className="mt-8">
              ...
              <p className={`italic text-lg max-w-xl mt-8 inline-block ${dark ? "text-white": ""}`}>
